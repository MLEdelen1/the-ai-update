...
def generate_site():
    ...
    for i, s in enumerate(stories[:100]):
        ...
        # Default title and summary from feed
        ...
        if md_path.exists():
            ...
            # 1. Extract EXACT title from the generated Markdown file
            for line in lines:
                if line.strip().startswith('# '):
                    title = line.strip().replace('# ', '').strip()
                    break

            # 2. Extract overview from the first real paragraph if summary is empty/short
            if len(summary) < 20:
                for line in lines:
                    cl = line.strip()
        ...
        # Homepage Card uses EXACT title & real overview
