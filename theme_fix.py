...
# 1. Write master.html
Path('website/templates/master.html').write_text('''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body class="bg-slate-950 text-slate-200 antialiased selection:bg-cyan-500 selection:text-slate-900">
    <nav class="max-w-7xl mx-auto px-6 py-6 flex justify-between items-center border-b border-slate-800/50">
        ...
        <div class="hidden md:flex space-x-8 font-semibold text-slate-400">
            ...
            <a href="sponsors.html" class="hover:text-cyan-400 transition">Sponsor</a>
...
Path('website/templates/article.html').write_text('''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
sg = sg.replace(old_news, new_news)
sg = sg.replace(old_tools, new_tools)
...
sg = sg.replace(old_gen, new_gen)
sg_path.write_text(sg)
print("Theme fixed successfully!")
